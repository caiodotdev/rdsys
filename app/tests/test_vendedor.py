...
class VendedorPermission(TestCase):
    
    @staticmethod
    def perform_create():
        return models.Vendedor.objects.create(
            name="delete_me"
        )
    ...
    def test_unauthenticated_users_cant_create(self):
        response = self.unauthorized_client.get(reverse_lazy(conf.VENDEDOR_CREATE_URL_NAME))
        self.assertNotEqual(response.status_code, 200)

    def test_unauthenticated_users_cant_update(self):
        response = self.unauthorized_client.get(
            reverse_lazy(
                conf.VENDEDOR_UPDATE_URL_NAME,
    ...
    def test_unauthorized_users_cant_create(self):
        response = self.client.get(reverse_lazy(conf.VENDEDOR_CREATE_URL_NAME))
        self.assertNotEqual(response.status_code, 200)
    ...
    def test_authorized_users_can_create(self):
        create_permission = auth_models.Permission.objects.get(codename="add_vendedor")
        self.user.user_permissions.add(create_permission)
        response = self.client.get(reverse_lazy(conf.VENDEDOR_CREATE_URL_NAME))
        self.assertEqual(response.status_code, 200)

    def test_authorized_users_can_update(self):
        change_permission = auth_models.Permission.objects.get(codename="change_vendedor")
    ...
    def test_authorized_users_can_delete(self):
        delete_permission = auth_models.Permission.objects.get(codename="delete_vendedor")
